...
public class Buscar implements Task {
	...
	public static Task porPalabraClave (String palabra) {
		return Instrumented.instanceOf(Buscar.class).withProperties(palabra);
	}

	@Step("{0} buscar por palabra clave '#palabra'")
	public <T extends Actor> void performAs(T actor) {
		actor.attemptsTo(Enter.theValue(palabra).into(WikiPage.CASILLA_BUSQUEDA).thenHit(Keys.RETURN));
